...
void Insert (OrderedSet <WordData> & S);
void TestDereference (OrderedSet <WordData>::Iterator itr);
void TestSubscript(OrderedSet <WordData> & S, int sub);
void TestIteratorOperators(OrderedSet <WordData> & S, char a);
void TestIsIn(OrderedSet <WordData> & S, int value);
void TestIsEmpty(OrderedSet <WordData> & S);
void TestDelete(OrderedSet <WordData> & S, int value);


int main ()
{
  srand (0);
  OrderedSet <WordData> S1;
  OrderedSet <WordData> S2;
  //Insert (S1);
  //Insert (S2);
...
void TestIteratorOperators(OrderedSet <WordData> & S, char a)
{
        try
	  {
		...
		for (OrderedSet<WorddData>::Iterator itr = S.begin(); itr != S.end(); itr++)
		  {
	    ...
	    if (a == 'm')
	      {
		cout << "Testing -- Iterator operator:\n";
		for (OrderedSet<WordData>::Iterator itr = S.rbegin(); itr != S.rend(); itr--)
		  {
		    cout << "Number " << *itr << endl;
		  }
	      }
